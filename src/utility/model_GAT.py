...
class BaseModel(nn.Module):
    def __init__(self, config):
        ...
        self.word_embedding = nn.Embedding.from_pretrained(torch.from_numpy(pretrained_embeddings),freeze=True,padding_idx=138).float()
        self._init_weights()
...
class GRMM(BaseModel):
    def _init_weights(self):
        ...
        self.model = GNN(args=args_gat,
                           input_dim=args.qrl_len,
                           output_dim=args.topk,
                           hidden_dim=args.qrl_len,
                           gru_step = args_gat.steps,
                           dropout_p=args_gat.dropout)
    ...
    def forward(self, adj, mask, qrls_words, doc_words, doc_ids, test=False):
        ...
        outputs,_ = self.model(torch.tensor(feat).float().to(device) , torch.tensor(adj).float().to(device), torch.tensor(mask).float().to(device))

        feat = outputs.permute(0, 2, 1)
