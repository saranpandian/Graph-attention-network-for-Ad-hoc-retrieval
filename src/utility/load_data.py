...
class Data(object):
    def __init__(self, path, batch_size):
        ...
        train_files = ["../data/msmarco-passage/qrels/AAAI_Project.tsv"]
        valid_file = "../data/msmarco-passage/qrels/AAAI_Project_dev.tsv"
        test_file = "../data/msmarco-passage/qrels/AAAI_project_test.tsv"

        match_file = path + '/qrels.train.tsv'
        ...
        #         l = line.strip().split('\t')
        #         try:
        #            self.doc_dict[l[0]] = [int(x) for x in l[1].split()]
        #         except:
        #            self.doc_dict[l[0]] = [138]
        ...
        with open(match_file) as f:
            for l in tqdm(f.readlines()):
                qrl_key, doc_key, score = l.strip().split('\t')
        ...
        with open(test_file) as f_test:
            for l in tqdm(f_test.readlines()):
                if len(l) == 0: break
                l = l.strip()
                # qrl_key, _, doc_key, _, _, _ = l.split()
                qrl_key, doc_key, _ = l.split('\t')
        ...
        with open(valid_file) as f_valid: 
            for l in tqdm(f_valid.readlines()):
                if len(l) == 0: break
                l = l.strip()
                qrl_key, doc_key, _ = l.split('\t')
                qrl = self.qrl_dict[qrl_key]
