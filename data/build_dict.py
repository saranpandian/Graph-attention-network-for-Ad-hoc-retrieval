...
parser.add_argument('--dataset', default='msmarco-passage', help='dataset name: robust04/clueweb09')
...
for line in tqdm(qrls):
  ...
  for w in words:
    if w not in remap_words:
      remap_words[w] = len(remap_words)
    n += str(remap_words[w]) + ' '
  # remap_words['PAD'] = len(remap_words)+1
  new_qrls.append(n)
...
print("creating the necessary dicts")
